...
class BatchCombinedExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict, cnn_output_dim: int = 256):
        # TODO we do not know features-dim here before going over all the items, so put something there. This is dirty!
        super(BatchCombinedExtractor, self).__init__(observation_space, features_dim=1)
    ...
    def forward(self, observations: TensorDict) -> th.Tensor:
        ...
        for key, extractor in self.extractors.items():
            if len(observations[key].shape) in [3, 5]:
                n_batch, n_agent = observations[key].shape[:2]
                obs = observations[key].reshape(-1, *(observations[key].shape[2:]))
            else:
                obs = observations[key]
            encoded_tensor_list.append(extractor(obs))

        cat = th.cat(encoded_tensor_list, dim=1)
        
        return cat if not n_batch else cat.reshape((n_batch, n_agent, self.features_dim)) 
