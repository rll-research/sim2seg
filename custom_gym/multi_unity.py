...
class UnityToGymWrapper(gym.Env):
    ...
    def _single_step(self, info: Union[DecisionSteps, TerminalSteps]) -> GymStepResult:
        ...
        if self._get_n_vis_obs() >= 1:
            visual_obs = self._get_vis_obs_list(info)
            if self.final_eval:
                self.visual_obs = np.concatenate((visual_obs[1][0], visual_obs[0][0]), axis=1)
            else:
...
class MultiUnityToGymWrapper(gym.Env):
    """
    Provides Gym wrapper for Unity Learning Environments.
    """
    ...
    def _check_agents(n_agents: int) -> None:
        if n_agents <= 1:
            raise UnityGymException(
                f"Must be more than 1 agent for multi agent!"
            )
