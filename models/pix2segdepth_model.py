...
class Pix2SegDepthModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_Depth', type=float, default=1.0, help='weight for L1 loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_CE', 'G_L1', 'D_real', 'D_fake', 'G_Depth']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        self.visual_names = ['real_A', 'fake_B', 'real_B', 'fake_B_gumbel', 'fake_B_soft', 'real_Depth', 'fake_Depth']
        ...
        # segmentation and depth -> output_nc + 1
        self.netG = networks.define_G(opt.input_nc, opt.output_nc + 1, opt.ngf, opt.netG, opt.norm,
    ...
    def set_input(self, input):
        ...
        self.real_Depth = input['D'].to(self.device) / 255
    ...
    def set_val_input(self, input):
        ...
        self.val_real_Depth = input['D'].to(self.device) / 255
    ...
    def get_fake_AB(self, detach=False):
        if self.D_type == "gumbel":
            ...
            conv_seg = self.seg_network(probs - 0.5 + torch.randn_like(self.real_B) * 0.1) # normalize
    ...
    def get_real_AB(self):
        if self.D_type == "gumbel":
            ...
            conv_pix = self.pix_network(self.real_A)
            # normalize and add noise - won't add up to one for segmentation classes
            with torch.no_grad():
                noisy_real_B = self.real_B - 0.5 + torch.randn_like(self.real_B) * 0.1
            conv_seg = self.seg_network(noisy_real_B)
    ...
    def forward(self):
        ...
        self.fake_B, self.fake_Depth = out[:, :self.opt.output_nc, :, :], out[:, self.opt.output_nc:, :, :]
        self.fake_Depth = (self.fake_Depth + 1)/2
        # depth does not work for terrain (blue)
        depth_mask = 1 - self.real_B[:, 2, :, :].unsqueeze(1)
        with torch.no_grad():
            self.fake_Depth *= depth_mask
            self.real_Depth *= depth_mask
    ...
    def forward_val(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        with torch.no_grad():
            out = self.netG(self.val_real_A)  # G(A)
            self.val_fake_B, self.val_fake_Depth = out[:, :self.opt.output_nc, :, :], out[:, self.opt.output_nc:, :, :]
            self.val_fake_Depth = (self.val_fake_Depth + 1)/2
            # depth does not work for terrain (blue)
            depth_mask = 1 - self.val_real_B[:, 2, :, :].unsqueeze(1)
            with torch.no_grad():
                self.val_fake_Depth *= depth_mask
                self.val_real_Depth *= depth_mask
    ...
    def backward_G(self):
        ...
        self.loss_G_Depth = self.criterionDepth(self.fake_Depth, self.real_Depth) * self.opt.lambda_Depth

        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + self.loss_G_CE + self.loss_G_Depth
